...
db98cx8580-32cd_profile()
{
    MAC_ADDR=$(ip link show eth0 | grep ether | awk '{print $2}')
    find /usr/share/sonic/device/*db98cx8580_32cd* -name profile.ini | xargs sed -i "s/switchMacAddress=.*/switchMacAddress=$MAC_ADDR/g"
    echo "db98cx8580-32cd: Updating switch mac address ${MAC_ADDR}"
}
